...
class IntentPlugin: MethodCallHandler, FlutterPlugin, ActivityAware {
    ...
    private var activityProvider: ActivityPluginBinding? = null
    ...
    override fun onMethodCall(call: MethodCall, result: Result) {
        ...
        activityProvider!!.addActivityResultListener { requestCode, resultCode, intent ->
            when (requestCode) {
                999 -> {
        ...
        when (call.method) {
            ...
            "startActivity" -> {
                Log.d("TAG", "startActivity Executed")
                val intent = Intent()
                ...
                try {
                    if (call.argument<Boolean>("chooser")!!) activityProvider!!.activity.startActivity(Intent.createChooser(intent, "Sharing"))
                    else activityProvider!!.activity.startActivity(intent)
                } catch (e: Exception) {
            ...
            "startActivityForResult" -> {
                Log.d("TAG", "startActivityForResult Executed")
                ...
                try {
                    if (intent.action == MediaStore.ACTION_IMAGE_CAPTURE) {
                        intent.resolveActivity(activityProvider!!.activity.packageManager).also {
                            getImageTempFile()?.also {
                                tobeCapturedImageLocationFilePath = it
                                activityProvider!!.activity.packageName
                                toBeCapturedImageLocationURI = FileProvider.getUriForFile(activityProvider!!.activity.applicationContext, "${activityProvider!!.activity.packageName}.io.github.itzmeanjan.intent.fileProvider", it)
                                intent.putExtra(MediaStore.EXTRA_OUTPUT, toBeCapturedImageLocationURI)
                                activityProvider!!.activity.startActivityForResult(intent, activityImageVideoCaptureCode)
                            }
                        }
                        ...
                        if (call.argument<Boolean>("chooser")!!) activityProvider!!.activity.startActivityForResult(Intent.createChooser(intent, "Sharing"), activityIdentifierCode)
                        else activityProvider!!.activity.startActivityForResult(intent, activityIdentifierCode)
                    }
                } catch (e: Exception) {
                    result.error("Error", e.toString(), null)
                }
    ...
    private fun getImageTempFile(): File? {
        return try {
            ...
            val storageDir = activityProvider!!.activity.getExternalFilesDir(Environment.DIRECTORY_PICTURES)
    ...
    private fun getVideoTempFile(): File? {
        return try {
            val timeStamp = SimpleDateFormat("ddMMyyyy_HHmmss", Locale.getDefault()).format(Date())
            val storageDir = activityProvider!!.activity.getExternalFilesDir(Environment.DIRECTORY_DCIM)
    ...
    private fun resolveContacts(uri: Uri): String {
        ...
        activityProvider!!.activity.applicationContext.contentResolver.query(uri, arrayOf(ContactsContract.CommonDataKinds.Phone.NUMBER), null, null, null).apply {
            this?.moveToFirst()
    ...
    private fun uriToFilePath(uri: Uri): String {
        val cursor = activityProvider!!.activity.applicationContext.contentResolver.query(uri,
